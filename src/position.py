...
def element_positions(
    ...
    element_height, element_width = element_shape
    ...
    while attempts < max_attempts and (valid_positions or positions_to_try):
        ...
        x1, y1 = x0 + element_width, y0 + element_height

        # Extract the region where element would be placed
        region_availability = availability_mask[y0:y1, x0:x1]
        region_masked = mask[y0:y1, x0:x1]

        # Check if the element fits image at this position
        ...
        padded_y0, padded_y1 = y0 - element_padding[1], y1 + element_padding[1]
        padded_x0, padded_x1 = x0 - element_padding[0], x1 + element_padding[0]
        ...
        if type == "sampling":
            ...
            neighbors = [
                # Top row
                (padded_x0 - element_width, padded_y0 - element_height),  # NW
                (padded_x0,                 padded_y0 - element_height),  # N
                (padded_x0 + element_width, padded_y0 - element_height),  # NE

                # Middle row (left and right of current rect)
                (padded_x0 - element_width, padded_y0),           # W
                (padded_x0 + element_width, padded_y0),           # E

                # Bottom row
                (padded_x0 - element_width, padded_y0 + element_height),  # SW
                (padded_x0,                 padded_y0 + element_height),  # S
                (padded_x0 + element_width, padded_y0 + element_height),  # SE
            ]
    ...
    print(
        f"Placed {elements_placed} elements" + f"in {attempts} attempts, " +
